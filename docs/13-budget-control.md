...
- **任务 4**：在 Agent 主循环迭代前插入预算超额检查，并在 REPL 侧实现 `/cost` 指令和 `show_cost` 展示。
- **任务 5**：实现 Tier 1 级别的上下文压缩机制——根据上下文负载比例截断超长工具执行结果。
...
#### 做什么
打开 `python/mini_claude/__main__.py`，修改 `_parse_args` 函数添加命令行参数；同时在 `main` 函数实例化 `Agent` 时将参数传入：

```python
# python/mini_claude/__main__.py -> _parse_args()
...
```python
# python/mini_claude/__main__.py -> main()
...
#### 做什么
打开 `python/mini_claude/agent.py`。在常量区添加计费常量（对应 Claude 3.5 Sonnet 的标准计费）：

```python
# python/mini_claude/agent.py -> Constants

COST_PER_INPUT_TOKEN = 3 / 1_000_000   # 每百万输入 Token 3.00 美元
COST_PER_OUTPUT_TOKEN = 15 / 1_000_000  # 每百万输出 Token 15.00 美元
```

在 `AgentState` 类中，确保追踪如下 Token 和轮数变量（这些字段应在构造时或每轮 API 交互后累加更新）：

```python
# python/mini_claude/agent.py -> AgentState
...
#### 做什么
在 `python/mini_claude/agent.py` 的 `Agent` 类中，编写 `_get_current_cost_usd` 和 `_check_budget` 两个核心助手方法：

```python
# python/mini_claude/agent.py -> Agent 类内部
...
#### 做什么
1. **主循环拦截**：在 `python/mini_claude/agent.py` 的 `chat` / `run_once` 的主 `while` 循环体中，发起 API 交互或执行工具前，立刻进行超限拦截判定：

```python
# python/mini_claude/agent.py -> run_once() / chat()
...
并在 `python/mini_claude/__main__.py` 的 REPL 命令处理分支中拦截 `/cost` 输入：

```python
# python/mini_claude/__main__.py -> run_repl()
...
#### 注意什么
拦截检查必须同时覆盖 `run_once`（单次命令模式）和 `chat`（交互式 REPL 模式）两条入口主循环路径。
...
#### 做什么
在 `python/mini_claude/agent.py` 中，根据当前上下文负载百分比（`utilization`）对大文本结果进行剪切。我们分别为 Anthropic 与 OpenAI 格式编写截断处理方法：
...
在执行 4 层上下文压缩（微观压缩/剪切陈旧工具结果等）前，优先调用此方法：

```python
# python/mini_claude/agent.py -> _on_api_success() 内的压缩入口
...
* **按负载动态截断**：掌握了根据上下文积压率动态计算“字符级预算”的算法，在系统高负载时自动裁剪工具输出以抑制上下文崩溃。
